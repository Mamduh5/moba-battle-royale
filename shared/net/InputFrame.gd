...
func normalized() -> InputFrame:
	...
	copy.input_sequence = max(0, input_sequence)
	copy.client_tick = max(0, client_tick)
	...
	copy.cast_requests.clear()
	for request in cast_requests:
		copy.cast_requests.append(request.duplicate(true))
	return copy
...
func to_dict() -> Dictionary:
	return {
		...
		"cast_requests": cast_requests.duplicate(true),
	}
...
static func from_dict(data: Dictionary) -> InputFrame:
	...
	for request in data.get("cast_requests", []):
		if typeof(request) == TYPE_DICTIONARY:
			frame.cast_requests.append(request.duplicate(true))
